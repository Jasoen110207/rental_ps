        ...
        <div class="pt-3 border-t-2 border-on-surface">
          @if ($isPending)
            <div class="grid grid-cols-2 gap-2">
              <button type="button" onclick="processRequestAction('{{ $req->id }}', 'reject', 'admin')" class="w-full py-2 bg-surface text-error border-2 border-on-surface font-headline-sm text-xs font-bold uppercase neo-shadow-sm btn-press hover:bg-red-50">
                Tolak
              </button>
              <button type="button" onclick="processRequestAction('{{ $req->id }}', 'approve', 'admin')" class="w-full py-2 bg-primary text-on-primary border-2 border-on-surface font-headline-sm text-xs font-bold uppercase neo-shadow-sm btn-press hover:bg-primary-container">
                Setujui
              </button>
            </div>
          @else
